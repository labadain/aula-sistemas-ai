...
1. **Komprende Intelijénsia Artifisiál (AI)** nia paradigma sira, abordajen simbóliku (ajente, peskiza, lójika) vs. abordajem bazeia ba dadus, no sistema modernu sira-ne'ebé bazeia ba Large Language Models (LLMs).
2. **Dezenvolve, aplika, no avalia modelu Machine Learning (ML)** ba tarefa regresaun, klasifikasaun, no clustering.
3. **Dezenvolve, aplika, no avalia modelu Deep Learning** inklui fine-tuning modelu pre-treinadu sira ba tarefa komún sira.
4. **Komprende Natural Language Processing** inklui tokenizasaun, embeddings, no arkitektura Transformer iha tarefa prosesamentu linguajen.
5. **Dezenvolve aplikasaun Generative IA** liu-liu Large Language Models (LLMs) no ninia téknika sira ba dezenvolvimentu aplikasaun IA nian, inklui in-context learning.
6. **Dezenvolve no implementa sistema IA** inklui pipeline integradu no ninia deployment ba iha ambiente produsaun.
7. **Avalia risku no aspetu étiku IA** hodi komprende konseitu bias, privasidade, étika, no impaktu sosial hosi sistema IA.
...
3. [**Módulu 3: Deep Learning**](module-3-deep-learning/module-3.html) sei kobre rede neuronál sira no ninia prosesu treinamentu (inklui backpropagation), no aplikasaun iha área hanesan vizaun komputasionál. Estudante sira sei halo implementasaun prátika uza PyTorch atu treina ka fine-tuning modelu simples ida.
...
Ekpozisaun no interasaun iha aula teórika hodi aborda konteúdu programátiku. Ezersísiu ba modelasaun, rezolusaun problema no programasaun aula teórika-prátika, nune'e mós liuhosi dezenvolvimentu projetu sira.
...
## Observações
Lian ensinu uza Tetun maibé materia, instrusaun, no orientasaun sei uza Tetun ho Inglés.